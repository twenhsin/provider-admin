...
| spacing.gap.btngroup-x | gap-btngroup_x | gap-200 | 8px | 按鈕之間的距離 |
| spacing.gap.heading-x | gap-heading_x | - | 4px | heading text上下的距離 |
| spacing.gap.objectgroup-x | gap-objectgroup_x | gap-300 | 12px | 組件左右的距離 |
| spacing.gap.section-y | gap-section_y | gap-600 | 24px | section上下間距 |
...
| spacing.padding.tag-x | padding-tag_x | p-300 | 12px | tag 左右 padding |
| spacing.padding.object-x | padding-object_x | p-400 | 16px | 組件：按鈕，input，dropdown等左右間距 |
...
.btn-solid {
  /* 清楚表達設計意圖 */
  @apply gap-gap-icontext;        /* icon 和文字間距 */
  @apply px-padding-object-x;     /* 按鈕左右內距 */
  @apply h-button-md;             /* 標準按鈕高度 */
...
.btn-solid {
  ...
  @apply px-400;     /* = padding-object-x = 16px */
  @apply h-800;      /* = button-md = 40px */
}
```
